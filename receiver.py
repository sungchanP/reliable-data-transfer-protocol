...
def start_receiver(connection_ID, loss_rate=0.0, corrupt_rate=0.0, max_delay=0.0):
    ...
    recv_message = clientSocket.recv(1024).decode('utf-8')          # receive packet from sender
    total_packet_recv+=1
    
    ack = recv_message[0]                       # set ACK to seq# of received packet from sender
    nak = '0' if ack == '1' else '1'            # set NAK as opposite of ACK
    
    while True:

        if(checksum_verifier(recv_message)):                                            # packet is not corrupted, create packet with ACK
            exceptChecksum = '  {}                      '.format(ack)                   # packet without checksum
            msgChecksum = checksum(exceptChecksum)                                      
            ACKpacket = '  {}                      {}'.format(ack, msgChecksum)         # packet with checksum
        
        else:                                                                           # packet is corrupted, create packet with NAK
            total_corrupted_pkt_recv+=1
            exceptChecksum = '  {}                      '.format(nak)
            msgChecksum = checksum(exceptChecksum)
            ACKpacket = '  {}                      {}'.format(nak, msgChecksum)
        
        clientSocket.send(ACKpacket.encode('utf-8'))            # send packet to sender
        total_packet_sent+=1
        
        recv_message = clientSocket.recv(30).decode('utf-8')        # receive next packet from sender
        if recv_message == "":                                      # if sender side signals end, break loop and close socket
            break
        total_packet_recv+=1
        ack = recv_message[0]                               # set ACK to seq# of received packet from sender
        nak = '0' if ack == '1' else '1'                    # set NAK as opposite of ACK
